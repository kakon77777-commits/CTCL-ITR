...
def test_module_cli_emits_canonical_trajectory_summary():
    ...
    for subject in ('autonomy','governance'):
        ...
        assert q['supported_domain_trajectory']['supported_cell_counts']==[6,8,6]
        assert q['supported_domain_trajectory']['direction_reversal_count']==1
        assert q['stability_boundary_trajectories']['positive']['lineages'][0]['velocity_direction_reversal_count_by_family']['code']==1
...
def test_package_metadata_is_v0214_and_exposes_entry_point():
    text=(ROOT/'pyproject.toml').read_text(encoding='utf-8')
    assert 'version = "0.2.14"' in text
    ...
    assert '__version__ = "0.2.14"' in init_text
