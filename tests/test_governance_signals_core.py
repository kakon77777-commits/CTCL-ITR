...
def test_reference_horizon_gap_and_six_directional_breaches():
    report = analyze_governance_signals(metrics_report=_metrics(), horizon_assessment=_horizon(), policy=default_signal_policy())
    ...
    assert set(signals) == {"autonomy_governance_gap","effective_oversight_density_low","oversight_debt_high","escalation_latency_p95_high","explicit_intervention_deadline_breach","critical_oversight_coverage_low"}
...
def test_clear_profile_has_no_escalation():
    ...
    report = analyze_governance_signals(metrics_report=metrics, horizon_assessment=_horizon(autonomy=5.0, governance=8.0), policy=default_signal_policy())
    assert report["horizon"]["autonomy_governance_gap"] == -3.0
    assert report["horizon"]["governance_margin"] == 3.0
...
def test_invalid_horizon_contract_is_rejected():
    with pytest.raises(GovernanceSignalError, match="reliability_p"):
        analyze_governance_signals(metrics_report=_metrics(), horizon_assessment=_horizon(reliability=1.1), policy=default_signal_policy())
    with pytest.raises(GovernanceSignalError, match="nonnegative"):
        analyze_governance_signals(metrics_report=_metrics(), horizon_assessment=_horizon(autonomy=-1.0), policy=default_signal_policy())
    bad_unit = _horizon()
    bad_unit["measurement_contract"]["unit"] = "milliseconds"
    with pytest.raises(GovernanceSignalError, match="interaction_depth"):
        analyze_governance_signals(metrics_report=_metrics(), horizon_assessment=bad_unit, policy=default_signal_policy())


def test_inputs_are_not_mutated():
    metrics = _metrics(); horizon = _horizon(); policy = default_signal_policy()
    originals = (deepcopy(metrics), deepcopy(horizon), deepcopy(policy))
    analyze_governance_signals(metrics_report=metrics, horizon_assessment=horizon, policy=policy)
