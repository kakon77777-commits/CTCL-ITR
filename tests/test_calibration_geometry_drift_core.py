    ...
    for subject in ("autonomy", "governance"):
        ...
        for code in (0.7, 0.8):
            ...
            graph["nodes"].append(
                {
                    "cell_key": key,
                    "reference_family_weights": {"code": code, "research": research},
                    "band_sign_class": "positive_band",
                }
            )
        # Existing cells 0.4 and 0.5 become stable-positive.
        for code in (0.4, 0.5):
            research = round(1 - code, 1)
            key = f"code={code}|research={research}"
            node_map[key]["band_sign_class"] = "positive_band"
        graph["supported_node_count"] = 8
        graph["connected_components"] = [
            {
                "component_id": "supported:0000",
                "size": 8,
                "members": [n["cell_key"] for n in graph["nodes"]],
            }
        ]
        graph["isolated_nodes"] = []

        # Move stability boundary left.
        q["boundaries"]["positive_stability_zero_crossings"] = [
            {
                "cell_a": "code=0.3|research=0.7",
                "cell_b": "code=0.4|research=0.6",
                "endpoint_field": "band.lower",
                "endpoint_value_a": -0.1,
                "endpoint_value_b": 0.1,
                "interpolation_fraction": 0.5,
                "estimated_reference_family_weights": {"code": 0.35, "research": 0.65},
                "observed_cell": False,
            }
        ]
        q["boundaries"]["support_edges"] = [
            {
                "supported_cell": "code=0.8|research=0.2",
                "unsupported_cell": "code=0.9|research=0.1",
                "transfer_mass": 0.1,
            }
        ]

        # Existing five gradient edges remain, first changes; two appear.
        q["local_gradients"][0]["point_estimate_slope"] += 0.5
        q["local_gradients"].append(
            {
                "cell_a": "code=0.6|research=0.4",
                "cell_b": "code=0.7|research=0.3",
                "transfer_mass": 0.1,
                "increased_family": "code",
                "decreased_family": "research",
                "point_estimate_slope": 3.0,
                "band_lower_slope": 2.0,
                "band_upper_slope": 4.0,
            }
        )
        q["local_gradients"].append(
            {
                "cell_a": "code=0.7|research=0.3",
                "cell_b": "code=0.8|research=0.2",
                "transfer_mass": 0.1,
                "increased_family": "code",
                "decreased_family": "research",
                "point_estimate_slope": 3.5,
                "band_lower_slope": 2.5,
                "band_upper_slope": 4.5,
            }
        )
    return current
...
def test_support_domain_motion_and_sign_migration_are_separated():
    ...
    report = compare_surface_geometry(base, current, drift_spec())
    ...
    assert domain["gained_supported_cells"] == [
        "code=0.7|research=0.3",
        "code=0.8|research=0.2",
    ]
...
def _component_geometry(component_members, *, geometry_id):
    ...
    for subject in ("autonomy", "governance"):
        ...
        q["supported_graph"]["connected_components"] = [
            {"component_id": f"supported:{i:04d}", "size": len(members), "members": list(members)}
...
def test_component_overlap_reports_splits_and_merges_without_claiming_phase_transition():
    a = "code=0.1|research=0.9"
    b = "code=0.2|research=0.8"
    c = "code=0.3|research=0.7"
    d = "code=0.4|research=0.6"
    ...
    report = compare_surface_geometry(base, current, drift_spec())
    comp = report["subjects"]["autonomy"]["component_motion"]
...
def test_geometry_contract_mismatch_is_rejected():
    ...
    bad["measurement_contract"]["scope_id"] = "scope:other"
    with pytest.raises(CalibrationGeometryDriftError, match="measurement_contract"):
        compare_surface_geometry(base, bad, drift_spec())

    bad = deepcopy(current)
    bad["grid"]["grid_step"] = 0.2
    with pytest.raises(CalibrationGeometryDriftError, match="grid"):
        compare_surface_geometry(base, bad, drift_spec())

    with pytest.raises(CalibrationGeometryDriftError, match="method"):
        compare_surface_geometry(base, current, drift_spec(method="other"))
