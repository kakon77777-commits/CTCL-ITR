...
def test_module_cli_emits_canonical_drift_report():
    cp = run(
        ...
        "--base-geometry",
        str(EXAMPLES / "calibration_surface_geometry_report.json"),
        "--current-geometry",
        str(EXAMPLES / "calibration_surface_geometry_report_later.json"),
        "--drift",
        str(EXAMPLES / "calibration_geometry_drift_spec.json"),
    )
    assert cp.returncode == 0, cp.stderr
    assert json.loads(cp.stdout) == json.loads(
        (EXAMPLES / "calibration_geometry_drift_report.json").read_text(encoding="utf-8")
    )
...
def test_package_metadata_preserves_v0213_geometry_drift_contract():
    text = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    match = re.search(r'^version = "(\d+)\.(\d+)\.(\d+)"$', text, re.MULTILINE)
    assert match
    assert tuple(map(int, match.groups())) >= (0, 2, 13)
    ...
    init_match = re.search(r'^__version__ = "(\d+)\.(\d+)\.(\d+)"$', init_text, re.MULTILINE)
    assert init_match
    assert tuple(map(int, init_match.groups())) >= (0, 2, 13)
