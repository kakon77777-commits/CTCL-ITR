...
def make_approval_request(**kwargs: Any) -> dict[str, Any]:
    required = [
        "approval_id", "run_id", "intent_id", "trigger_event_id", "requested_action",
        "target", "risk_class", "reason", "options", "evidence_refs",
        "authority_requested", "requested_at", "expires_at",
    ]
    for key in required:
    ...
    out = {"schema_version": "0.2.3", **deepcopy(kwargs), "status": "pending"}
    return out


def make_decision_receipt(**kwargs: Any) -> dict[str, Any]:
    required = [
        "decision_id", "approval_id", "run_id", "principal", "decision",
        "selected_option", "decided_at", "human_active_ms", "human_governance_ms",
    ]
    for key in required:
        if key not in kwargs:
            raise GovernanceError(f"missing {key}")
...
def make_authority_grant(**kwargs: Any) -> dict[str, Any]:
    required = [
        "authority_ref", "decision_id", "principal", "scope", "target",
        "issued_at", "expires_at", "max_uses", "revocable",
    ]
    for key in required:
        if key not in kwargs:
            raise GovernanceError(f"missing {key}")
...
def evaluate_resume_eligibility(
    ...
    authority_grant: dict[str, Any] | None,
    *,
    action: str,
    target: str,
    at: str,
    ...
    return {
        ...
        "reasons": reasons,
        "authority_ref": authority_grant.get("authority_ref"),
...
def demo_payload() -> dict[str, Any]:
    approval = make_approval_request(
        ...
        risk_class="medium",
        reason="external publish requires human authorization",
        options=["approve", "deny"],
        evidence_refs=["validation:gov-001"],
        authority_requested={"scope": ["publish"], "max_uses": 1},
        requested_at="2026-08-20T08:00:00+00:00",
        expires_at="2026-08-20T09:00:00+00:00",
    )
    receipt = make_decision_receipt(
        decision_id="decision:gov-001",
    ...
    authority = make_authority_grant(
        authority_ref="auth:gov-001",
