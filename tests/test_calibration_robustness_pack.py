...
def test_calibration_robustness_module_cli_emits_exact_reference_report():
    ...
    completed = subprocess.run(
        [
            ...
            "--base",
            "examples/calibration_snapshot_base.json",
            "--current",
            "examples/calibration_snapshot_current.json",
            "--spec",
            "examples/calibration_comparison_spec.json",
        ],
        cwd=ROOT,
        env=env,
        capture_output=True,
...
def test_calibration_robustness_validator_passes():
    completed = subprocess.run(
        [sys.executable, "validator/validate_robustness.py"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    ...
    assert "autonomy_observed_delta=" in completed.stdout
    assert "autonomy_adjusted_delta=" in completed.stdout
    assert "governance_observed_delta=" in completed.stdout
    assert "governance_adjusted_delta=" in completed.stdout
