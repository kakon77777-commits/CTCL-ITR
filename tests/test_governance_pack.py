...
def load_json(name): return json.loads((EXAMPLES / name).read_text(encoding="utf-8"))
def load_schema(name): return json.loads((SCHEMAS / name).read_text(encoding="utf-8"))
...
def test_governance_reference_objects_validate_and_link():
    ...
    for obj, schema_name in [(approval,"approval-request.schema.json"),(decision,"decision-receipt.schema.json"),(grant,"authority-grant.schema.json")]:
...
def test_governance_reference_atl_events_form_auditable_handoff():
    events = [json.loads(line) for line in (EXAMPLES / "governance_checkpoint.events.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
    types = [e["event_type"] for e in events]
    assert types == ["human.checkpoint.requested","run.suspended","human.checkpoint.resolved","authority.checked","run.resumed"]
