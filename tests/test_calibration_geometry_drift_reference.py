...
def build_later_geometry():
    surface = analyze_joint_uncertainty_surface(load("calibration_snapshot_base.json"),load("calibration_snapshot_later.json"),load("calibration_comparison_spec_later.json"),load("calibration_joint_surface_spec_later.json"))
...
def test_geometry_drift_schemas_are_valid_draft_2020_12():
    for name in ("calibration-geometry-drift-spec.schema.json","calibration-geometry-drift-report.schema.json"):
...
def test_canonical_later_geometry_and_drift_artifacts_validate():
    ...
    Draft202012Validator(geometry_schema,format_checker=Draft202012Validator.FORMAT_CHECKER).validate(load("calibration_surface_geometry_report_later.json"))
    Draft202012Validator(drift_spec_schema,format_checker=Draft202012Validator.FORMAT_CHECKER).validate(load("calibration_geometry_drift_spec.json"))
    Draft202012Validator(drift_report_schema,format_checker=Draft202012Validator.FORMAT_CHECKER).validate(load("calibration_geometry_drift_report.json"))
...
def test_canonical_geometry_drift_report_is_exact_regeneration():
    fresh = compare_surface_geometry(load("calibration_surface_geometry_report.json"),build_later_geometry(),load("calibration_geometry_drift_spec.json"))
...
def test_canonical_geometry_drift_reference_summary():
    ...
    auto,gov = report["subjects"]["autonomy"],report["subjects"]["governance"]
    for subject in (auto,gov):
        ...
        assert subject["support_frontier_motion"]["matched_supported_endpoints"][0]["signed_family_displacement"]["code"] > 0
